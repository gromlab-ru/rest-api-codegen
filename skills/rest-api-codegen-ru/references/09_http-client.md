...
Формат выбирается по успешному ответу и применяется также к ответам с другими статусами. Если форматы успешного ответа и ошибки различаются, настройте content-type-aware `responseParser` на экземпляре `HttpClient`. Одна custom operation может изменить только общий `format` запроса и сама по себе не разделяет parsing успешного и ошибочного statuses.
...
Всегда ограничивайте количество повторов через `context.retryCount` и учитывайте идемпотентность метода. Для изменяющего запроса создайте idempotency key до первой попытки и повторяйте тот же key с неизменным body. Не генерируйте key внутри `onRequest`: interceptor выполняется заново при каждом `context.retry()`. Повтор безопасен только при подтверждённой server-side deduplication policy. Готовый пример: [ошибки, повтор запросов и отмена](./recipes/react-vite/errors-retry-cancellation.md).
